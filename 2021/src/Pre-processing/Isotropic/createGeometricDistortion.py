...
class createGeometricDistortion:
    ...
    def update_M(self):
        ...
        Ry = np.array([[math.cos(self.beta), 0, -math.sin(self.beta)],
                      [0, 1, 0], [math.sin(self.beta), 0, math.cos(self.beta)]])
        Rz = np.array([[math.cos(self.gamma), -math.sin(self.gamma), 0],
                      [math.sin(self.gamma), math.cos(self.gamma), 0], [0, 0, 1]])
        self.R = np.matmul(Rx, np.matmul(Ry, Rz))
        self.K = np.array([[-self.focus/self.sx, self.sh, self.ox],
                          [0, self.focus/self.sy, self.oy], [0, 0, 1]])
        self.M1 = np.array(
    ...
    def project(self, src):
        ...
        try:
            ...
            r_4 = r_2**2
            r_6 = r_2**3
            K = (1+self.KpCoeff[0]*r_2+self.KpCoeff[1]*r_4+self.KpCoeff[4]*r_6) / \
                ((1+self.KpCoeff[5]*r_2+self.KpCoeff[6]
                 * r_4+self.KpCoeff[7]*r_6))
            x_2 = x_1*K + 2*self.KpCoeff[2]*x_1 * \
                y_1 + self.KpCoeff[3]*(r_2+2*x_1**2)
            y_2 = y_1*K + self.KpCoeff[2] * \
                (r_2 + 2*y_1**2) + 2*self.KpCoeff[3]*x_1*y_1
