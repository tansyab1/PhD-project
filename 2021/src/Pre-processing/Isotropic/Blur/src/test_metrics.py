...
def get_gt_image(path):
    ...
    img = cv2.cvtColor(cv2.imread(os.path.join(
        base, 'sharp', seq, filename)), cv2.COLOR_BGR2RGB)
    return img
...
def test_image(model, image_path):
    ...
    img_tensor = torch.from_numpy(np.transpose(
        img_s / 255, (2, 0, 1)).astype('float32'))
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    filenames = sorted(glob.glob(args.img_folder + '/test' +
                       '/blur/**/*.png', recursive=True))
