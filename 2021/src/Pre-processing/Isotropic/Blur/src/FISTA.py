...
class FISTA(object):
    ...
    def __init__(self, p, lam, FGPIters, bmin=-np.inf, bmax=np.inf, FGPClass=FGPCython, monotone=True, nthreads=None):
        self.fgp = FGPClass(lam, FGPIters, p.vshape,
                            bmin=bmin, bmax=bmax, nthreads=nthreads)
        self.p = p
    ...
    def reconstruct(self, sinogram, iterations, x0=None, progress=False):
        ...
        for i in range(iterations):
            ...
            if progress:
                ...
                six.print_("{:.2f} % done ({}:{:02d}:{:02d})".format(
                    100*float(i)/iterations, hours, minutes, secs))
        return rec
