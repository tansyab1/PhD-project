...
sigma = 25
# 46 # default 2.7*sigma(default 25)           # Threshold for Hard Thresholding
...
# default 3                    # Rather than sliding by one pixel to every next reference block, use a step of Nstep pixels in both horizontal and vertical directions.
Step1_Blk_Step = 3
Step1_Search_Step = 3    # default 3               # 块的搜索step
# 39 default             # Search for candidate matching blocks in a local neighborhood of restricted size NS*NS centered
...
def Step1_fast_match_color(_noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    Final_similar_blocks = numpy.zeros(
        (max_matched, Blk_Size, Blk_Size, chnl), dtype=float)
    ...
    for ch in range(chnl):
        img = _noisyImg[present_x: present_x+Blk_Size,
                        present_y: present_y+Blk_Size, ch]
        dct_img[:, :, ch] = cv2.dct(img.astype(
            numpy.float64))  # 对目标作block作二维余弦变换

        Final_similar_blocks[0, :, :, ch] = dct_img[:, :, ch]

    blk_positions[0, :] = _BlockPoint

    Window_location = Define_SearchWindow(
    ...
    similar_blocks = numpy.zeros(
        (blk_num**2, Blk_Size, Blk_Size, chnl), dtype=float)
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            for ch in range(chnl):
                tem_img = _noisyImg[present_x: present_x +
                                    Blk_Size, present_y: present_y+Blk_Size, ch]
                dct_Tem_img[:, :, ch] = cv2.dct(tem_img.astype(numpy.float64))
            m_Distance = numpy.linalg.norm(
                (dct_img[:, :, 0]-dct_Tem_img[:, :, 0]))**2 / (Blk_Size**2)  # only on luminance

            # 下面记录数据自动不考虑自身(因为已经记录)
            if m_Distance < Threshold and m_Distance > 0:  # 说明找到了一块符合要求的
                for ch in range(chnl):
                    similar_blocks[matched_cnt, :, :,
                                   ch] = dct_Tem_img[:, :, ch]
    ...
    if Count > 0:
        for i in range(1, Count):
            for ch in range(chnl):
                Final_similar_blocks[i, :, :,
                                     ch] = similar_blocks[Sort[i-1], :, :, ch]
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            for ch in range(chnl):
                ...
                tem_Vct_Trans[numpy.abs(tem_Vct_Trans[:])
                              < Threshold_Hard3D] = 0.
    ...
    *对于最后的数组，乘以凯撒窗之后再输出
    ...
    for ch in range(chnl):
        ...
        for i in range(Count):
            ...
            tem_img = (1./_nonzero_num[ch]) * \
                cv2.idct(_similar_blocks[i, :, :, ch]) * Kaiser
            m_basic_img[point[0]:point[0]+_shape[1],
                        point[1]:point[1]+_shape[2], ch] += tem_img
...
def BM3D_1st_step_color(_noisyImg):
    ...
    print("step 1: Width_num = " + str(Width_num) +
    ...
    for i in range(int(Width_num+2)):
        for j in range(int(Height_num+2)):
            ...
            # 该函数用于保证当前的blk不超出图像范围
            m_blockPoint = Locate_blk(
                i, j, blk_step, block_Size, width, height)
            Similar_Blks, Positions, Count = Step1_fast_match_color(
                _noisyImg, m_blockPoint)
            Similar_Blks, statis_nonzero = Step1_3DFiltering_color(
                Similar_Blks)
            Aggregation_hardthreshold_color(
                Similar_Blks, Positions, Basic_img, m_Wight, statis_nonzero, Count, m_Kaiser)
    for ch in range(chnl):
...
def Step2_fast_match_color(_Basic_img, _noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    Final_similar_blocks = numpy.zeros(
        (max_matched, Blk_Size, Blk_Size, chnl), dtype=float)
    Final_noisy_blocks = numpy.zeros(
        (max_matched, Blk_Size, Blk_Size, chnl), dtype=float)
    dct_img = numpy.zeros((Blk_Size, Blk_Size, chnl), dtype=float)
    ...
    for ch in range(chnl):
        img = _Basic_img[present_x: present_x +
                         Blk_Size, present_y: present_y+Blk_Size, ch]
        dct_img[:, :, ch] = cv2.dct(img.astype(
            numpy.float64))  # 对目标作block作二维余弦变换
        Final_similar_blocks[0, :, :, ch] = dct_img[:, :, ch]

        n_img = _noisyImg[present_x: present_x +
                          Blk_Size, present_y: present_y+Blk_Size, ch]
        dct_n_img[:, :, ch] = cv2.dct(
            n_img.astype(numpy.float64))  # 对目标作block作二维余弦变换
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            for ch in range(chnl):
                tem_img = _Basic_img[present_x: present_x +
                                     Blk_Size, present_y: present_y+Blk_Size, ch]
                dct_Tem_img[:, :, ch] = cv2.dct(tem_img.astype(numpy.float64))
            m_Distance = numpy.linalg.norm(
                (dct_img-dct_Tem_img))**2 / (Blk_Size**2)
    ...
    if Count > 0:
        for i in range(1, Count):
            ...
            for ch in range(chnl):
                n_img = _noisyImg[present_x: present_x +
                                  Blk_Size, present_y: present_y+Blk_Size, ch]
                Final_noisy_blocks[i, :, :, ch] = cv2.dct(
                    n_img.astype(numpy.float64))
...
def Step2_3DFiltering_color(_Similar_Bscs, _Similar_Imgs, Count):
    ...
    *返回的Wiener_wight用于后面Aggregation
    ...
    Wiener_wight = numpy.zeros(
        (m_Shape[1], m_Shape[2], m_Shape[3]), dtype=float)
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            for ch in range(chnl):
                ...
                if m_weight != 0:
                    Wiener_wight[i, j, ch] = 1. / \
                        (m_weight**2 * sigma_color[ch]**2)
                else:
...
def Aggregation_Wiener_color(_Similar_Blks, _Wiener_wight, blk_positions, m_basic_img, m_wight_img, Count, Kaiser):
    ...
    *对于最后的数组，乘以凯撒窗之后再输出
    ...
    for ch in range(chnl):
        for i in range(Count):
            point = blk_positions[i, :]
            tem_img = _Wiener_wight[:, :, ch] * \
                cv2.idct(_Similar_Blks[i, :, :, ch])  # * Kaiser
...
def BM3D_2nd_step_color(_basicImg, _noisyImg):
    ...
    print("step 2: Width_num = " + str(Width_num) +
          ", Height_num = " + str(Height_num))
    ...
    for i in range(int(Width_num+2)):
        for j in range(int(Height_num+2)):
            m_blockPoint = Locate_blk(
                i, j, blk_step, block_Size, width, height)
            Similar_Blks, Similar_Imgs, Positions, Count = Step2_fast_match_color(
                _basicImg, _noisyImg, m_blockPoint)
            Similar_Blks, Wiener_wight = Step2_3DFiltering_color(
                Similar_Blks, Similar_Imgs, Count)
            # print(Similar_Blks.shape)
            Aggregation_Wiener_color(
                Similar_Blks, Wiener_wight, Positions, m_img, m_Wight, Count, m_Kaiser)
    for ch in range(chnl):
        m_img[:, :, ch] /= m_Wight[:, :, ch]
...
def PSNR2(img1, img2):
    D = numpy.array(img2, dtype=numpy.int64) - \
        numpy.array(img1, dtype=numpy.int64)
    D = D**2
