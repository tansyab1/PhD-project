...
def gabor_filter(img, filters):
    ...
    for kern in filters:
        ...
        gabor_img += cv2.filter2D(img, cv2.CV_64F, kern, borderType=cv2.BORDER_REFLECT)
    return gabor_img/4

def jnc_plus(img, filter=None, name=None):
    illmask = cv2.medianBlur(img, 201)
    gabor_img=gabor_filter(img, filter)
    BsMat = np.zeros(img.shape)
    image_global = cv2.copyMakeBorder(img, 3, 3, 3, 3, cv2.BORDER_REFLECT)
    ...
    # save the image to a file in the folder
    cv2.imwrite(folder_link+"/jnc_plus_"+name+".png", BMat*255)
...
def jnc(img, d=1, name=None):
    ...
    image = cv2.copyMakeBorder(img, 1, 1, 1, 1, cv2.BORDER_REFLECT)
