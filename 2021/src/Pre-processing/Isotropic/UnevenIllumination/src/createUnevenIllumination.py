...
class createUnevenIllumination:
    ...
    def create_mask(self, given_size, center, gradient_shape):
        ...
        # mask
        mask_left = [left_corner[i] - ori_left[i]
                     for i in range(len(left_corner))]
        mask_right = [right_corner[i] - ori_right[i] +
                      gradient_shape for i in range(len(right_corner))]
    ...
    def createUnevenIllumination(self, image, center, theta=40):
        if self.mode == 'circle':
            mask = self.create_mask([np.shape(image)[0], np.shape(
                image)[1]], center, self.gradient_shape[0])+255-self.max_intensity
        elif self.mode == 'oval':
        ...
        image[:, :, 0] = image[:, :, 0] * \
            self.transparency + mask * (1 - self.transparency)
        image[:, :, 1] = image[:, :, 1] * \
            self.transparency + mask * (1 - self.transparency)
        image[:, :, 2] = image[:, :, 2] * \
            self.transparency + mask * (1 - self.transparency)

        hsv[:, :, 2] = np.where(hsv[:, :, 2] > 30, hsv[:, :, 2] *
                                self.transparency + mask * (1 - self.transparency), hsv[:, :, 2])
        lab[:, :, 0] = np.where(lab[:, :, 0] > 30, lab[:, :, 0] *
                                self.transparency + mask * (1 - self.transparency), lab[:, :, 0])
...
if __name__ == "__main__":
    ...
    centers = [[100, 100], [np.shape(frame)[
        0]//2, np.shape(frame)[1]//2], [np.shape(frame)[0]-100, np.shape(frame)[1]-100]]
    ...
    for max_intensity in max_intensities:
        for center in centers:
            frame = cv2.imread('results/test.jpg')
            file_name = 'UnevenIllumination'+'_mode:circle' + \
                '_level:'+str(max_intensity)+'_center:'+str(center)
            p = createUnevenIllumination(
                circle_light_shape, max_intensity, transparency, mode='circle')
    ...
    for max_intensity in max_intensities:
        for center in centers:
            for theta in thetas:
                frame = cv2.imread('results/test.jpg')
                file_name = 'UnevenIllumination'+'_mode:oval'+'_level:' + \
                    str(max_intensity)+'_center:' + \
                    str(center)+'_theta:'+str(theta)
                p = createUnevenIllumination(
                    circle_light_shape, max_intensity, transparency, mode='oval')
                image, hsv_res, lab_res = p.createUnevenIllumination(
                    frame, center, theta)
