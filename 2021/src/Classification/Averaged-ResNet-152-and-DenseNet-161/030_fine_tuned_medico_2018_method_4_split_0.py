...
parser.add_argument("--val_fold", type=str, default="split_0",
                    help="Select the validation fold", choices=["fold_1", "fold_2", "fold_3"])
parser.add_argument("--all_folds", default=["split_0", "split_1"],
...
class MyNet(nn.Module):
    def __init__(self, num_out=23):
        ...
        self.densenet_model.classifier = nn.Linear(
            self.densenet_num_ftrs, num_out)
        checkpoint_densenet = torch.load(opt.best_densenet)
        self.densenet_model.load_state_dict(
            checkpoint_densenet["model_state_dict"])  # Best densenet
