...
train_dataset = get_training_data(
    train_dir, {'patch_size': opt.TRAINING.TRAIN_PS})
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_char_item = [criterion_char(restored[j], target)
                          for j in range(len(restored))]
        loss_edge_item = [criterion_edge(restored[j], target)
                          for j in range(len(restored))]
        loss_char = torch.stack(loss_char_item).sum(dim=0)
        loss_edge = torch.stack(loss_edge_item).sum(dim=0)
