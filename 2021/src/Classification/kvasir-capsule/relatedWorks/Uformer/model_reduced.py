    ...
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        if cross_modulator:
            self.cross_modulator = nn.Embedding(
                win_size*win_size, dim)  # cross_modulator
        ...
        if token_mlp in ['ffn', 'mlp']:
            ...
            self.mlp = LeFF(dim, mlp_hidden_dim,
                            act_layer=act_layer, drop=drop)

        elif token_mlp == 'fastleff':
            self.mlp = FastLeFF(dim, mlp_hidden_dim,
                                act_layer=act_layer, drop=drop)
        else:
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            input_mask_windows = window_partition(
                input_mask, self.win_size)  # nW, win_size, win_size, 1
        ...
        if self.shift_size > 0:
            ...
            shift_mask_windows = window_partition(
                shift_mask, self.win_size)  # nW, win_size, win_size, 1
            # nW, win_size*win_size
            shift_mask_windows = shift_mask_windows.view(
                -1, self.win_size * self.win_size)
            shift_attn_mask = shift_mask_windows.unsqueeze(
                1) - shift_mask_windows.unsqueeze(2)  # nW, win_size*win_size, win_size*win_size
            shift_attn_mask = shift_attn_mask.masked_fill(
                shift_attn_mask != 0, float(-100.0)).masked_fill(shift_attn_mask == 0, float(0.0))
...
class Uformer(nn.Module):
    ...
    def flops(self):
        ...
        flops += self.encoderlayer_2.flops()+self.dowsample_2.flops(self.reso //
                                                                    2**2, self.reso//2**2)
        flops += self.encoderlayer_3.flops()+self.dowsample_3.flops(self.reso //
                                                                    2**3, self.reso//2**3)
...
if __name__ == "__main__":
    ...
    print('# model_restoration parameters: %.2f M' % (sum(param.numel()
          for param in model_restoration.parameters()) / 1e6))
