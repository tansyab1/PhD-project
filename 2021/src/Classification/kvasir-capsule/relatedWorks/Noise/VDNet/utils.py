...
def batch_PSNR(img, imclean, border=0):
    ...
    for i in range(Img.shape[0]):
        PSNR += calculate_psnr(Iclean[i, :, ].transpose((1, 2, 0)),
                               Img[i, :, ].transpose((1, 2, 0)), border)
...
def batch_SSIM(img, imclean, border=0):
    ...
    SSIM = 0
    for i in range(Img.shape[0]):
        SSIM += calculate_ssim(Iclean[i, :, ].transpose((1, 2, 0)),
                               Img[i, :, ].transpose((1, 2, 0)), border)
...
def generate_gauss_kernel_mix(H, W):
    ...
    ZZ = 1./(2*np.pi*scale**2) * \
        np.exp((-(XX-centerW)**2-(YY-centerH)**2)/(2*scale**2))
...
class LogGamma(autoF):
    ...
    def forward(ctx, input):
        ...
        out = torch.from_numpy(out).to(
            device=input.device).type(dtype=input.dtype)
