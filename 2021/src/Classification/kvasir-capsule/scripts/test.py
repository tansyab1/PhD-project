...
def generalized_gaussian_ratio(alpha):
    return (special.gamma(2.0/alpha)**2) / (special.gamma(1.0/alpha) * special.gamma(3.0/alpha))
...
def estimate_aggd_params(x):
    ...
    stddev_right = math.sqrt((1.0/(x_right.size - 1))
                             * numpy.sum(x_right ** 2))
    if stddev_right == 0:
    ...
    beta_left = stddev_left * \
        math.sqrt(special.gamma(3.0/alpha) / special.gamma(1.0/alpha))
    beta_right = stddev_right * \
        math.sqrt(special.gamma(3.0/alpha) / special.gamma(1.0/alpha))
...
def compute_features(img_norm):
    ...
    for x_shift, y_shift in ((0, 1), (1, 0), (1, 1), (1, -1)):
        img_pair_products = img_norm * \
            numpy.roll(numpy.roll(img_norm, y_shift, axis=0), x_shift, axis=1)
        alpha, beta_left, beta_right = estimate_aggd_params(img_pair_products)
        eta = (beta_right - beta_left) * \
            (special.gamma(2.0/alpha) / special.gamma(1.0/alpha))
...
def normalize_image(img, sigma=7/6):
    ...
    mu_sq = mu * mu
    sigma = numpy.sqrt(numpy.abs(gaussian_filter(
        img * img, sigma, mode='nearest') - mu_sq))
    img_norm = (img - mu) / (sigma + 1)
    return img_norm


def niqe(img):
    model_mat = scipy.io.loadmat('./data/modelparameters.mat')
    ...
    for scale in [1, 2]:
        ...
        for block_col in range(img_norm.shape[0]//block_size):
            for block_row in range(img_norm.shape[1]//block_size):
                block_features = compute_features(img_norm[block_col*block_size:(
                    block_col+1)*block_size, block_row*block_size:(block_row+1)*block_size])
    ...
    niqe_quality = math.sqrt(
        (model_mu - features_mu).dot(pseudoinv_of_avg_cov.dot((model_mu - features_mu).T)))

    return niqe_quality


img = imageio.imread("test.png").astype(numpy.float)/255.0
