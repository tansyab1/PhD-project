...
with tf.name_scope("cross_ent"):
    net_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        logits=score, labels=y) + loss_L2)
...
def test(test_image_batch, test_label_batch):
    ...
    for i in range(num_test // batch_size):
        ...
        pred, att_map, loss, acc = sess.run(
            [prediction, att_maps, net_loss, accuracy], feed_dict=feed_dict)
        ...
        for index in range(batch_size):
            ...
            save_img(att_map[index], img_index, att_path,
                     img_name='.jpg', mode="heatmap")
