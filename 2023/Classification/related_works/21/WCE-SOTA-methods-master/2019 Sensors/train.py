...
inception_input = tf.image.resize_images(x, (224, 224))
y = tf.placeholder(tf.float32, [None, num_classes])
keep_prob = tf.placeholder(tf.float32)
is_training = tf.placeholder(tf.bool, shape=[])

# The first model is Alexnet
# net1 = alexnet(x, num_classes, is_training = True, dropout_keep_prob = keep_prob)
net1 = AlexNet(x, keep_prob, num_classes)
logits1 = net1.fc8
...
def load_model():
    saver = tf.train.Saver()
    save_path = 'save_model/model.chkpt'
    try:
