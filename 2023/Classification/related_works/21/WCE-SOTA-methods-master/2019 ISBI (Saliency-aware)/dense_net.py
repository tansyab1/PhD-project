...
class DenseNet:
    ...
    def deconv_block(self, _input, kernel_size, out_features, unpool='True',
        ...
        deconv = tf.nn.conv2d_transpose(
            un_relu, filter, output_shape, strides, padding, data_format='NHWC')
    ...
    def compute_saliency(self, f_maps):
        ...
        s_map = tf.div(s_map - s_map_min, s_map_max -
                       s_map_min)  # [batch_size, 8, 8, 1]

        saliency_map = tf.tile(s_map, (1, 1, 1, 3))
        saliency_map = tf.image.resize_images(
            saliency_map, (224, 224))  # (8, 224, 224, 3)
    ...
    def DGN_module(self, _input):
        ...
        unpool1 = self.max_unpool(deconv3, self.max_ind, [
                                  None, 112, 112, 48], self.batch_size, "unpool")

        with tf.variable_scope("deconv4"):
            deconv4 = self.deconv_block(
                unpool1, kernel_size=7, out_features=24, unpool='False', strides=[1, 2, 2, 1])
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("net1") as scope:
            ...
            self.input2 = tf.concat([self.images, tf.reduce_mean(
                self.s_map1, axis=-1, keepdims=True)], axis=-1)
    ...
    def train_all_epochs(self, train_params):
        ...
        try:
            ...
            for epoch in range(1, n_epochs + 1):
                ...
                print("Training...")
                loss1, loss2, acc1, acc2 = self.train_one_epoch(
                    batch_size, learning_rate)
                if self.should_save_logs:
                    self.log_loss_accuracy(
                        loss1, loss2, acc1, acc2, acc2, epoch, prefix='train')
                ...
                if train_params.get('validation_set', False):
                    ...
                    loss1, loss2, acc1, acc2, acc, nr1, br1, ir1, kappa1, nr2, br2, ir2, \
                        kappa2, nr, br, ir, kappa = self.test(
                            weight_1, weight_2, batch_size)

                    if self.should_save_logs:
                        self.log_loss_accuracy(
                            loss1, loss2, acc1, acc2, acc, epoch, prefix='valid')
                    ...
                    weight_1 = np.exp(weight1) / \
                        (np.exp(weight1) + np.exp(weight2))
                    weight_2 = np.exp(weight2) / \
                        (np.exp(weight1) + np.exp(weight2))
    ...
    def train_one_epoch(self, batch_size, learning_rate):
        ...
        for i in range(self.num_train // batch_size):
            ...
            fetches = [self.train_step, self.cross_entropy1,
                       self.cross_entropy2, self.accuracy1, self.accuracy2]
    ...
    def test(self, w1, w2, batch_size):
        ...
        for i in range(self.num_test // batch_size):
            test_images, test_labels = self.sess.run(
                [self.test_image_batch, self.test_label_batch])
            ...
            loss1, loss2, avg_s1, s_map1, s_map2, pred1, pred2, pred = self.sess.run(
            ...
            for index in range(batch_size):
                ...
                save_img(test_images[index], img_index,
                         input_path, img_name='.jpg', mode="image")
                save_img(s_map1[index], img_index, saliency_path,
                         img_name='_saliency.jpg', mode="heatmap")
                save_img(avg_s1[index], img_index, saliency_path,
                         img_name='_avg_saliency.jpg', mode="heatmap")
                # plt.figure()
