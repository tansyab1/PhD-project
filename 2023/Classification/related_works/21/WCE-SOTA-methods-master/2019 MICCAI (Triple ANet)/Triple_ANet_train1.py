...
FLAGS = flags.FLAGS
flags.DEFINE_string(
    "tfdata_path", '/home/xxh/Documents/Journal/dataset/sum_train_test_only/split1/', "tf-records save path")
...
def bottleneck_layer_2d(input, filters, drop_rate, decay, scope, training, reuse):
    with tf.variable_scope(name_or_scope=scope, reuse=reuse):
        x = batch_norm(input, reuse, is_training=training,
                       scope=scope + '_batch1')
...
def DTOA(x, channels, de=4, scope='DTOA', trainable=True, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        h = conv(x, channels, kernel=1, stride=1, sn=SN,
        ...
        correlation = tf.nn.softmax(
            correlation/(tf.sqrt(tf.cast(c_num, tf.float32))), axis=-1)  # (8, hw, hw)
...
def triple_anet(image, labels, label_count, drop_rate=1.0, decay=0.9, growth_k=12, trainable=True, reuse=False, scope='dis'):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        logits = nonlocal_block(logits, int(
            logits.shape[-1]), de=4, scope='NL0', trainable=trainable, reuse=reuse)
        print(logits)
        ...
        logits = nonlocal_block(logits, int(
            logits.shape[-1]), de=4, scope='NL1', trainable=trainable, reuse=reuse)
        print(logits)
        ...
        logits = nonlocal_block(logits, int(
            logits.shape[-1]), de=4, scope='NL2', trainable=trainable, reuse=reuse)
        print(logits)
...
train_image_batch, train_label_batch = utils.get_image_label_batch(
    config, shuffle=True, name='train')
test_image_batch, test_label_batch = utils.get_image_label_batch(
    config, shuffle=False, name='4aug_test')
        ...
        for epoch in range(1, 60+1):
            ...
            for batch_idx in range(batch_count):
                ...
                _, loss, acc, tr_end_points, GG = sess.run(
                    [d_optim, cross_entropy, train_accuracy,
                        train_end_points, Gamma],
                ...
                if batch_idx == 0:
                    ...
                    save_images(test_att0[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_att0.jpg')
                    save_images(test_att1[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_att1.jpg')
                    save_images(test_att2[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_att2.jpg')
                    save_images(b4_att[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_att_b4.jpg')
            ...
            if epoch >= 35 and epoch % 5 == 0:
                save_path = saver.save(sess, os.path.join(
                    './logs/' + model_name + '/model/', 'nonlocal'+'.model'), global_step=epoch)
        infile.close()
