...
def deconv(x, channels, kernel=4, stride=2, padding='SAME', use_bias=True, sn=False, scope='deconv_0'):
    with tf.variable_scope(scope):
        ...
        if padding == 'SAME':
            output_shape = [x_shape[0], x_shape[1] *
                            stride, x_shape[2] * stride, channels]

        else:
            output_shape = [x_shape[0], x_shape[1] * stride + max(
...
def deform_conv(x, kernel_size, stride, output_channals, mode):
    if mode == 'offset':
        layer_output = tf.keras.layers.Conv2D(filters=kernel_size*kernel_size*2, kernel_size=kernel_size, strides=stride, padding='SAME',
                                              kernel_initializer=tf.constant_initializer(0.0), bias_initializer=tf.zeros_initializer())(x)
        layer_output = tf.clip_by_value(
            layer_output, -0.25*int(x.shape[1]), 0.25*int(x.shape[1]))
    if mode == 'weight':
        layer_output = tf.keras.layers.Conv2D(filters=kernel_size*kernel_size*output_channals, kernel_size=kernel_size, strides=stride, padding='SAME',
                                              kernel_initializer=tf.constant_initializer(0.5), bias_initializer=tf.zeros_initializer())(x)
    if mode == 'feature':
        ...
        layer_output = conv(x, output_channals, kernel=kernel_size,
                            stride=kernel_size, sn=True, scope='feature')
    return layer_output
...
def reshape_x_offset(x_offset, kernel_size):
    ...
    x_offset = [tf.reshape(x_offset[:, :, :, s:s+kernel_size, :], new_shape)
                for s in range(0, N, kernel_size)]
...
def batch_norm(x, is_training=True, scope='batch_norm'):
    ...
    return tf.keras.layers.BatchNormalization()(x, training=is_training)
...
def class_loss(class_logits, label, num_class):
    ...
    loss = tf.losses.softmax_cross_entropy(
        tf.one_hot(label, num_class), class_logits, weights=1.0)
