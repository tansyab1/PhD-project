...
from utilsForTF import Data_set
# import sys
...
def conv2pool(input, filters, kernel, decay, stride, scope, training, reuse=True):
    x = conv(input, channels=filters, kernel=kernel, stride=stride,
             pad=1, sn=SN, use_bias=False, scope=scope)
...
def bottleneck_layer_2d(input, filters, drop_rate, decay, scope, training, reuse):
    with tf.compat.v1.variable_scope(name_or_scope=scope, reuse=reuse):
        x = batch_norm(input, training, scope=scope + '_batch1')
        x = tf.nn.relu(x)
        x = conv(x, channels=4 * filters, kernel=1, stride=1,
                 pad=0, sn=SN, use_bias=False, scope=scope+'_conv1')
        #x = tf.contrib.layers.dropout(inputs=x, keep_prob=drop_rate, is_training=training)

        x = batch_norm(x, training, scope=scope + '_batch2')
        x = tf.nn.relu(x)
        x = conv(x, channels=filters, kernel=3, stride=1, pad=1,
                 sn=SN, use_bias=False, scope=scope+'_conv2')
        #x = tf.contrib.layers.dropout(inputs=x, keep_prob=drop_rate, is_training=training)
        return x
...
def transition_layer_2d(input, filters, drop_rate, decay, scope, training, reuse):
    with tf.compat.v1.variable_scope(name_or_scope=scope, reuse=reuse):
        ...
        x = tf.nn.relu(x)
        x = conv(x, channels=filters, kernel=1, stride=1, pad=0,
                 sn=SN, use_bias=False, scope=scope+'_conv')
        #x = tf.contrib.layers.dropout(x, keep_prob=drop_rate, is_training=training)
        x = tf.keras.layers.AveragePooling2D(
            pool_size=2, strides=2, padding='SAME')(x)
        return x
    ...
    with tf.name_scope(scope):
        ...
        for i in range(nb_layers):
            ...
            gamma = tf.compat.v1.get_variable(
                scope+"gamma"+str(i), [1], initializer=tf.constant_initializer(1.0))
...
def AAM(x, channels, de=4, scope='AAM', trainable=True, reuse=False):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        f, offset1 = deform_con2v(x, num_outputs=channels // de, kernel_size=3,
                                  stride=1, trainable=trainable,  name=scope+'f_conv', reuse=reuse)
        g, offset2 = deform_con2v(x, num_outputs=channels // de, kernel_size=3,
                                  stride=1, trainable=trainable,  name=scope+'g_conv', reuse=reuse)
        h = conv(x, channels, kernel=1, stride=1, sn=SN, scope='h_conv')

        # N = h * w
        s = tf.linalg.matmul(hw_flatten(g), hw_flatten(
            f), transpose_b=True)  # [bs, N, N]

        beta_a = tf.nn.softmax(s, dim=-1)  # attention map

        o = tf.linalg.matmul(beta_a, hw_flatten(h))  # [bs, N, C]
...
def Squeeze_excitation_layer(input, out_dim, ratio, scope='squeeze_excitation', reuse=False):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        excitation = tf.keras.layers.Dense(
            units=out_dim, activation='sigmoid')(excitation)
...
def g_theta(arccos, k):
    ...
    sigmoid2 = (1-tf.exp(arccos/k-math.pi/2.0/k)) / \
        (1+tf.exp(arccos/k-math.pi/2.0/k))
...
def ACLoss(embedding, labels, out_num, w_init=None, s=64., m=0.5, k=0.3, is_training=True):
    ...
    with tf.compat.v1.variable_scope('ACLoss'):
        ...
        arccos = tf.math.acos(tf.linalg.matmul(
            embedding, weights, name='cos_t'))
        ...
        A = [1] * out_num
        exclude = tf.linalg.tensor_diag(A)
        zeros = array_ops.zeros_like(exclude, dtype=exclude.dtype)
        reg = tf.linalg.matmul(weights, weights, transpose_a=True)
        reg = tf.where(exclude > 0.0, zeros, reg)
        regularization = tf.reduce_sum(reg) / (out_num * (out_num - 1))

        if is_training:
            arccos_mt = tf.acos(
                tf.linalg.matmul(embedding, weights, name='cos_t')) + m
            ...
            inv_mask = tf.math.subtract(1., mask, name='inverse_mask')

            s_cos_t = tf.math.multiply(s, cos_t, name='scalar_cos_t')

            output = tf.math.add(tf.math.multiply(s_cos_t, inv_mask),
                                 tf.math.multiply(cos_mt_temp, mask), name='arcface_loss_output')
...
def triple_anet(image, labels, label_count, drop_rate=1.0, decay=0.9, growth_k=12, trainable=True, reuse=False, scope='dis'):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        # print(logits)
        offset1, offset2, logits = AAM(logits, int(
            logits.shape[-1]), de=4, scope='AAM0', trainable=trainable, reuse=reuse)
        # print(logits)
        ...
        #################################################################################
        logits = dense_block_2d(logits, growth_k, nb_layers=12, drop_rate=drop_rate,
                                decay=decay, training=trainable, reuse=reuse, scope='dense_block_2')
        # print(logits)
        logits = transition_layer_2d(logits, filters=0.5*int(logits.shape[-1]),  drop_rate=drop_rate, decay=decay,
                                     training=trainable, reuse=reuse, scope='trans_layer_2')
        # print(logits)
        offset1, offset2, logits = AAM(logits, int(
            logits.shape[-1]), de=4, scope='AAM1', trainable=trainable, reuse=reuse)
        ...
        #################################################################################
        logits = dense_block_2d(logits, growth_k, nb_layers=24,  drop_rate=drop_rate,
                                decay=decay, training=trainable, reuse=reuse, scope='dense_block_3')
        # print(logits)
        logits = transition_layer_2d(logits, filters=0.5*int(logits.shape[-1]),  drop_rate=drop_rate, decay=decay,
        ...
        #################################################################################
        logits = dense_block_2d(logits, growth_k, nb_layers=16, decay=decay,
                                drop_rate=drop_rate, training=trainable, reuse=reuse, scope='dense_block_4')
        # print(logits)
        ...
        # print(feature)
        # with AC Loss
        pred, ys_, reg, regularization = ACLoss(
            logits, labels, label_count, w_init=w_init, s=64., m=0.5, k=0.3, is_training=trainable)
...
xs = tf.compat.v1.placeholder(
    tf.float32, shape=[batch_size, 128, 128, 3], name='images')
ys = tf.compat.v1.placeholder(tf.int32, shape=[batch_size], name='label')
lr = tf.compat.v1.placeholder("float", shape=[])

data_train = "./downstream_folds/train/"
data_val = "./downstream_folds/val/"
Data_train = Data_set(config, shuffle=True, name='train3', path=data_train)
Data_val = Data_set(config, shuffle=False, name='val3', path=data_val)
...
l2 = tf.add_n([weight_decay * tf.nn.l2_loss(var)
              for var in tf.compat.v1.trainable_variables()])
d_optim = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True).minimize(
    cross_entropy + l2 + regularization)
...
val_correct_prediction = tf.equal(
    tf.cast(tf.argmax(val_pred, 1), tf.int32), ys)
...
with tf.compat.v1.Session(config=sess_config) as sess:
    ...
    try:
        ...
        train_image_batch, train_label_batch = data_train.next_iter()
        train_images, train_label = sess.run(
            [train_image_batch, train_label_batch])
        GG = sess.run(Gamma, feed_dict={
                    xs: train_images, ys: train_label, lr: learning_rate})
        for k in GG.keys():
        ...
        for epoch in range(1, 1+100):
            ...
            for batch_idx in range(batch_count):
                ...
                train_images, train_label = sess.run(
                    [train_image_batch, train_label_batch])
                _, loss, acc, tr_end_points, GG = sess.run([d_optim, cross_entropy, train_accuracy, train_end_points, Gamma],
                ...
                if batch_idx % 50 == 0:
                    ...
                    for batch_idx in range(batch_count):
                        counter1 += 1
                        test_image_batch, test_label_batch = data_val.next_iter()
                        test_images, test_label = sess.run(
                            [test_image_batch, test_label_batch])
                        test_results, te_end_points = sess.run(
                            [test_accuracy, val_end_points], feed_dict={xs: test_images, ys: test_label})
                    ...
                    print("offset:", test_off0_0.max(), test_off0_1.max(),
                          test_off1_0.max(), test_off1_1.max())
                    infile.write(str(test_off0_0.max())+' '+str(test_off0_1.max()) +
                                 ' '+str(test_off1_0.max())+' '+str(test_off1_1.max())+' \n')
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(
                    './logs/'+model_name+'/model/', 'triple_anet'+'.model'), global_step=epoch)
        infile.close()
        ...
        for k in GG.keys():
            if 'dense_block_' in k and 'gamma' in k:
                GGamma_key = k.split(':')[0]
                GGamma_key = GGamma_key.split('/')[1]
                gamma_file = open('./logs/'+model_name +
                                  '/'+GGamma_key+'.txt', 'w')
