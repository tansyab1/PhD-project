...
def conv2pool(input, filters, kernel, decay, stride, scope, training, reuse=True):
    ...
    x = tf.contrib.layers.max_pool2d(
        inputs=x, kernel_size=[2, 2], stride=2, padding='VALID')
    return x
...
def transition_layer_2d(input, filters, drop_rate, decay, scope, training, reuse):
    with tf.variable_scope(name_or_scope=scope, reuse=reuse):
        ...
        x = tf.contrib.layers.avg_pool2d(
            inputs=x, kernel_size=[2, 2], stride=2, padding='VALID')
        return x
...
def Squeeze_excitation_layer(input, out_dim, ratio, scope='squeeze_excitation', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        excitation = tf.layers.dense(
            squeeze, units=out_dim / ratio, name='fully_connected1')
        excitation = tf.nn.relu(excitation)
        excitation = tf.layers.dense(
            excitation, units=out_dim, name='fully_connected2')
...
def ACLoss(embedding, labels, out_num, w_init=None, s=64., m=0.5, k=0.3, is_training=True):
    ...
    with tf.variable_scope('ACLoss'):
        ...
        if is_training:
            arccos_mt = tf.acos(
                tf.matmul(embedding, weights, name='cos_t')) + m
            ...
            output = tf.add(tf.multiply(s_cos_t, inv_mask), tf.multiply(
                cos_mt_temp, mask), name='ACLoss_output')
...
train_image_batch, train_label_batch = utils.get_image_label_batch(
    config, shuffle=True, name='train3')
test_image_batch, test_label_batch = utils.get_image_label_batch(
    config, shuffle=False, name='test3')
    ...
    try:
        ...
        for epoch in range(1, 1+100):
            ...
            for batch_idx in range(batch_count):
                ...
                train_images, train_label = sess.run(
                    [train_image_batch, train_label_batch])
                _, loss, acc, tr_end_points, GG = sess.run([d_optim, cross_entropy, train_accuracy, train_end_points, Gamma], feed_dict={
                                                           xs: train_images, ys: train_label, lr: learning_rate})
                ...
                if batch_idx == 0:
                    ...
                    save_images(test_off0_0[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_off0_0.jpg')
                    save_images(test_off0_1[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_off0_1.jpg')
                    save_images(test_off1_0[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_off1_0.jpg')
                    save_images(test_off1_1[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_off1_1.jpg')
                    save_images(test_att0[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_AAM0.jpg')
                    save_images(test_att1[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test_AAM1.jpg')
                    save_images(test_images[:m_h * m_w, :, :, :], [m_h, m_w], './logs/' +
                                model_name+'/saliency/'+str(epoch)+'_'+str(batch_idx)+'test.jpg')
