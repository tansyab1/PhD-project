...
class TrainingWithSAM:
    ...
    def runner(self):
        ...
        optimizer = SAM(self.model.parameters(
        ), base_optimizer=self.optimizer, lr=1, momentum=0.9, weight_decay=0.0001)
        scheduler = CyclicLR(optimizer, base_lr=1e-06,
                             max_lr=0.1, step_size_up=50, mode='triangular2')
        train_losses = []
        train_accu = []
        test_losses = []
        test_accu = []
        ...
        trainTestPlot(self.plot, train_accu, test_accu,
                      train_losses, test_losses, self.model_name+"with_SAM_")
