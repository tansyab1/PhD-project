    ...
    def read_processing_generate_image_label_batch(self):
        if self.name.find('train') != -1:
            ...
            filename_queue = tf.train.string_input_producer(
                tfrecord_filename, num_epochs=None, shuffle=True)
            ...
            image_batch = tf.contrib.image.rotate(image_batch, tf.random_uniform(shape=(tf.shape(
                image_batch)[0], ), minval=-0.5, maxval=0.5, seed=37), interpolation='BILINEAR')
            ...
            filename_queue = tf.train.string_input_producer(
                tfrecord_filename, num_epochs=None, shuffle=False)
            # get tensor of image/label
            image, label = read_tfrecord_and_decode_into_image_label_pair_tensors(filename_queue,
                                                                                  self.actual_image_size)
            #image = channels_image_standardization(image)
        ...
        image_batch = tf.image.resize_bilinear(
            image_batch, size=[self.actual_image_size, self.actual_image_size])
