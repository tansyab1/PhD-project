...
def DA_loss(Features, labels, out_num, s=64., m=0.5, k=0.3, scope='arc_loss', is_cross=True, reuse=True):
    ...
    with tf.variable_scope(scope+'arc_loss', reuse=reuse):
        ...
        embedding = tf.div(Features, embedding_norm +
                           10e-6, name='norm_embedding')
    ...
    with tf.variable_scope(scope+'center_loss', reuse=reuse):
        ...
        centers = tf.get_variable(
            'centers', dtype=tf.float32, initializer=tf.transpose(weights), trainable=False)

        ###### regularization term calculate##########
        centers_norm = tf.norm(centers, axis=1, keep_dims=True)
        centers_normed = tf.div(centers, centers_norm +
                                10e-6, name='norm_weights')
        A = [1] * out_num
...
def _tower_loss(network_fn, images, labels, is_cross=True, reuse=False, is_training=False):
    ...
    for i in range(FLAGS.num_networks):
        ...
        if is_cross:
            net_pred["{0}".format(i)], net_logits["{0}".format(i)], net_regularization["{0}".format(i)], weight, c_update_op["{0}".format(i)] = DA_loss(
            ...
            net_pred["{0}".format(i)] = layers.softmax(
                net_pred["{0}".format(i)], scope='predictions')

            raw_loss = tf.nn.softmax_cross_entropy_with_logits(
                logits=net_logits["{0}".format(i)], labels=labels)
            ...
            print('semi_data!')
            net_pred["{0}".format(i)], _, _, _, _ = DA_loss(net_features["{0}".format(i)], tf.argmax(
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('tower') as scope:
            with tf.variable_scope(tf.get_variable_scope()):
                test_net_loss, _, test_net_pred, test_attention0, test_attention1, test_second_input = _tower_loss(
                    network_fn, test_image_batch, test_label_batch, is_cross=True, reuse=False, is_training=False)
                ...
                for i in range(FLAGS.num_networks):
                    ...
                    test_precision["{0}".format(i)] = tf.reduce_mean(tf.to_float(
                        tf.equal(test_predictions["{0}".format(i)], test_truth)))
                    #test_predictions["{0}".format(i)] = test_net_pred["{0}".format(i)]
                net_pred = (test_net_pred["{0}".format(
                    0)] + test_net_pred["{0}".format(1)])/2.0
        ...
        load_fn = slim.assign_from_checkpoint_fn(os.path.join(
            FLAGS.checkpoint_dir, 'model.ckpt-0'), tf.global_variables(), ignore_missing_vars=True)
        ...
        for batch_idx in range(batch_count):
            # for i in range(FLAGS.num_networks):
            test_predictions_value["{0}".format(0)], test_predictions_value["{0}".format(1)], truth, predictions, test_precision_value["{0}".format(0)], test_precision_value["{0}".format(1)], prec, test, test_att0, test_att1, test_sec = sess.run(
                [test_predictions["{0}".format(0)], test_predictions["{0}".format(1)], test_truth, net_pred, test_precision["{0}".format(0)], test_precision["{0}".format(1)], precision_mean, test_image_batch, test_attention0, test_attention1, test_second_input])
            ...
            infile.write(str(test_predictions_value["{0}".format(
                0)])+' '+str(test_predictions_value["{0}".format(1)])+'\n')
            infile.write(str(predictions)+' '+str(truth)+'\n')
            infile.write(str(np.float32(test_precision_value["{0}".format(0)]))+' '+str(
                np.float32(test_precision_value["{0}".format(1)]))+' '+str(np.float32(prec))+'\n')
            format_str = 'batch_idx: [%3d] [%3d/%3d] time: %4.4f, net0_test_acc = %.4f,      net1_test_acc = %.4f,      net_test_acc = %.4f'
            print(format_str % (batch_idx, batch_idx, batch_count, time.time()-start_time, np.float32(
                test_precision_value["{0}".format(0)]), np.float32(test_precision_value["{0}".format(1)]), np.float32(prec)))
...
#                 test_att11 = to_heat(test_att11)
                scipy.misc.imsave(os.path.join(FLAGS.attention_map, str(
                    batch_idx)+'_'+str(index)+'test.jpg'), test1[:, :, :])
                scipy.misc.imsave(os.path.join(FLAGS.attention_map, str(
                    batch_idx)+'_'+str(index)+'test_att0.jpg'), test_att01[:, :, :])
                scipy.misc.imsave(os.path.join(FLAGS.attention_map, str(
                    batch_idx)+'_'+str(index)+'test_att1.jpg'), test_att11[:, :, :])
                ...
                scipy.misc.imsave(os.path.join(FLAGS.attention_map, str(
                    batch_idx)+'_'+str(index)+'test_sec.jpg'), test_sec1[:, :, :])
        ...
        infile.write(str(np.mean(np.array(precision_value["{0}".format(0)])))+' '+str(np.mean(np.array(
            precision_value["{0}".format(1)])))+' '+str(np.mean(np.array(precision_value["{0}".format(2)])))+'\n')
        infile.close()
