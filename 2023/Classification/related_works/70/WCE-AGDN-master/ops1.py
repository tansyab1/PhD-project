"""
This script contains some functions used by the AGDN model, such as the deformable conv, spectral norm...
"""
...
def batch_norm(x, reuse, is_training=True, scope='batch_norm'):
    # return tf.layers.batch_normalization(x, training=is_training)

    return tf.keras.layers.BatchNormalization(momentum=0.9,
                                              epsilon=1e-5,
                                              scale=True,
                                              trainable=True)(x, training=is_training)
