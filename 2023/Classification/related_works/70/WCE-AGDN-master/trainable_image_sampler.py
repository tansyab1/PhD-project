...
def gauss_conv_2d(_input, padding_size=30, strides=[1, 1, 1, 1], padding='VALID'):
    ...
    gauss_kernel = gaussian_weights[:, :,
                                    tf.newaxis, tf.newaxis]  # (61, 61, 1, 1)
...
def generate_structured_grid(saliency, batch_size, src_size, dst_size, grid_size=31, padding_size=30):
    ...
    dst_x = np.tile(dst_x[np.newaxis, :, :, np.newaxis], [
                    batch_size, 1, 1, 1])  # (8, 1, 91, 1)
    uniform_x = tf.convert_to_tensor(dst_x, dtype=tf.float32)

    dst_y = np.tile(dst_y[np.newaxis, :, :, np.newaxis], [
                    batch_size, 1, 1, 1])  # (8, 91, 1, 1)
    uniform_y = tf.convert_to_tensor(dst_y, dtype=tf.float32)

    # generate the saliency distribution along the x axis.
    saliency_x = tf.reduce_max(
        saliency, axis=1, keepdims=True)  # (8, 1, 91, 1)
    ...
    saliency_y = tf.reduce_max(
        saliency, axis=2, keepdims=True)  # (8, 91, 1, 1)
...
def generate_pixel_grid(saliency, batch_size, src_size, dst_size, grid_size=31, padding_size=30):
    ...
    uniform_x = tf.reshape(
        dst_coords[:, :, :, 0], [-1, global_size, global_size, 1])
    uniform_y = tf.reshape(
        dst_coords[:, :, :, 1], [-1, global_size, global_size, 1])
...
def get_resampled_images(image, saliency, batch_size, src_size, dst_size, padding_size=30, lamda=0.5):
    ...
    for i in range(padding_size):
        saliency = tf.pad(saliency, [[0, 0], [1, 1], [
                          1, 1], [0, 0]], 'symmetric')
    ...
    structured_sample_grids, structured_src_grids = generate_structured_grid(
        padded_saliency, batch_size, src_size, dst_size)  # (8, 128, 128, 2)
    pixel_wise_sample_grids, pixel_wise_src_grids = generate_pixel_grid(
        padded_saliency, batch_size, src_size, dst_size)  # (8, 128, 128, 2)

    sample_grids = (1.0 - lamda) * structured_sample_grids + \
        lamda * pixel_wise_sample_grids
