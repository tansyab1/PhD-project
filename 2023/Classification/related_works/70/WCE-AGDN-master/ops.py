"""
This script contains some functions used by the AGDN model.
...
def spatial_second_order_att(feature_maps):
    ...
    aggregated_feature = tf.reshape(
        aggregated_feature, [bs, f_size, f_size, c_num])
...
def channel_second_order_att(feature_maps):
    ...
    channel_att = tf.reshape(tf.reduce_mean(
        correlation, axis=-1), [bs, 1, 1, c_num])
...
def COAM_att(feature_maps):
    ...
    att_map = tf.reshape(tf.reduce_mean(
        correlation, axis=-1), [bs, f_size, f_size, 1])
...
def channel_cor_att(feature_maps):
    ...
    reweight_f = tf.reduce_mean(
        feature_maps * channel_att, axis=-1, keepdims=True)  # [bs, h, w, 1]
...
def co_attention_layer(f1, f2):
    ...
    with tf.variable_scope("Co_attention_layer"):
        ...
        original_att1 = tf.reduce_mean(
            f1, axis=-1, keepdims=True)  # [bs, w, h, 1]
...
def lesion_att_acc(img_index, gt_img, pixel_preds, pred_fg, prob, mode="Net1"):
    ...
    print(mode, "index", img_index, "Att_acc is:",
          att_acc, "Prob of the correct class is:", prob)
...
def lesion_center_acc(pixel_preds, gt_img):
    ...
    dist_error = round(np.sqrt(
        np.sum(np.square(np.array(fg_center_preds) - np.array(fg_center_gt)))), 2)
...
# Regular operations
####################################################
def save_img(img, img_index, root_path, img_name, split, mode="image"):
    img = np.uint8(255 * img)
    if mode == "image":
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    elif mode == "heatmap":
        img = cv2.applyColorMap(img, cv2.COLORMAP_JET)
    img_path = os.path.join(root_path, split + '_' + str(img_index) + img_name)
...
def generate_gt_map(gt_img, dst_gt_path, img_index, mode="Net1"):
    ...
    cv2.imwrite(os.path.join(dst_gt_path, str(
        img_index) + '.jpg'), np.uint8(255 * gt_img))
